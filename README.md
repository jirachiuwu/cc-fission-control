...
- **モニター GUI**: 温度・ダメージ・冷却材・加熱冷却材・燃料・廃棄物・タービンのバー、burn rate、状態バナー
...
|---|---|---|---|
| `safety` | 1000K | 1150K | マージン厚め。まず安全に |
| `balance` | 1100K | 1180K | 効率と安全の中間（既定）|
| `performance` | 1150K | 1190K | 高出力寄り（ソーダリウム高冷却前提）|

> 初回は `balance` で挙動を見て、問題なければ `performance` に上げるのを推奨。
