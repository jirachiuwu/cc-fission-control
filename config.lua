...
local cfg = {
  ...
  control = {
    ...
    -- ★最重要（その1）★ coolant をこの % 以上に保つ。低いほど高出力（runout に近い）。
    -- 冷却材供給が律速の炉ではこれで止まる。実機の coolant は循環で 7〜15% 程度に落ち着く。
    coolantSetpoint     = 12,
    -- ★最重要（その2）★ heated（加熱冷却材＝逆流蒸気）をこの % 以下に保つ。高いほど高出力だが
    -- タービン詰まり（蒸気パンパン→ボイラー水枯れ）に近づく。タービンが律速の炉ではこれで止まる。
    -- coolant 下限と heated 上限の「先に効く方」で自動的に頭打ちになる（二重制約）。
    heatedSetpoint      = 60,
