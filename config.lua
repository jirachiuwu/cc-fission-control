...
local cfg = {
  ...
  control = {
    ...
    -- 冷却系の余裕しきい値（復水/冷却が追いつかない兆候を温度より先に検知してメルトダウン予防）。
    -- coolant%（冷却材の残量）が下がる = 復水が追いついてない。heated%（加熱冷却材）が上がる = 蒸気が抜けてない。
    -- どちらかが stop を超えたら「温度が目標未満でも burn を上げない」（持続可能な点で頭打ちにする）。
    coolantStopPct      = 50,   -- coolant がこれ未満なら昇圧停止
    heatedStopPct       = 50,   -- heated がこれ超なら昇圧停止
    -- さらに悪化（backoff 到達）したら温度に関係なく burn を下げる。
    coolantBackoffPct   = 35,   -- coolant がこれ未満なら強制降圧
    heatedBackoffPct    = 80,   -- heated がこれ超なら強制降圧
  },
